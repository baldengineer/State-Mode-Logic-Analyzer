...
void smla_pio_setup() {
    ...
    pio_sm_set_consecutive_pindirs(pio, pio_sm, 8, 24, GPIO_IN);
    ...
    sm_config_set_in_shift(&c, false, true, 24); // i've setup 8 bits to make this boundry clean, i think
...
void smla_pio_run() {
    ...
    pio_sm_exec(pio, pio_sm, pio_encode_jmp(SMLA_offset_start));
